...
function pharmacien(variante) {
  ...
  function jambe(cote) {
    ...
    genou.add(mollet);
    const pied = boiteArrondie(0.15, 0.09, 0.27, 0.04, matChauss);
    pied.position.set(0, -0.41, 0.05);
    genou.add(pied);
    g.add(genou);
  ...
  torse.add(dos);
  // col
  const col = boiteArrondie(0.3, 0.1, 0.24, 0.045, matBlouse);
  col.position.set(0, 0.5, 0.02);
  torse.add(col);
  // badge
  ...
  torse.add(badge);
  // poche
  const poche = new THREE.Mesh(new THREE.BoxGeometry(0.13, 0.11, 0.01),
  ...
  function bras(cote) {
    ...
    coude.add(avant);
    const main = new THREE.Mesh(new THREE.SphereGeometry(0.068, 18, 14), matPeau);
    main.position.y = -0.3;
    main.scale.set(1, 1.18, 0.82);
  ...
  const crane = new THREE.Mesh(new THREE.SphereGeometry(0.175, 36, 26), matPeau);
  crane.scale.set(0.93, 1.06, 0.97);
  crane.castShadow = true;
  tete.add(crane);

  // menton légèrement marqué
  const menton = new THREE.Mesh(new THREE.SphereGeometry(0.1, 20, 16), matPeau);
  menton.position.set(0, -0.09, 0.03);
  ...
  const cheveux = new THREE.Mesh(
    new THREE.SphereGeometry(0.181, 36, 26, 0, Math.PI * 2, 0, Math.PI * 0.52), matCheveux);
  ...
  if (feminin) {
    ...
    [-1, 1].forEach(function (cote) {
      ...
      meche.position.set(cote * 0.135, -0.08, -0.02);
      meche.scale.set(0.62, 1.7, 0.95);
      meche.castShadow = true;
      tete.add(meche);
    });
    const arriere = new THREE.Mesh(new THREE.SphereGeometry(0.15, 24, 18), matCheveux);
  ...
  [-1, 1].forEach(function (cote) {
    ...
    o.position.set(cote * 0.162, -0.005, 0.005);
  ...
  nez.position.set(0, -0.018, 0.158);
  nez.scale.set(0.85, 1.15, 0.9);
  tete.add(nez);

  // yeux : blanc + iris, pour un regard net à petite taille
  [-1, 1].forEach(function (cote) {
    const blanc = new THREE.Mesh(new THREE.SphereGeometry(0.034, 18, 14), mat(0xFAFCFE, 0.35));
    blanc.position.set(cote * 0.066, 0.022, 0.146);
    blanc.scale.set(1, 0.78, 0.5);
    tete.add(blanc);
    const iris = new THREE.Mesh(new THREE.SphereGeometry(0.019, 14, 12), mat(0x1F3346, 0.3));
    iris.position.set(cote * 0.066, 0.020, 0.160);
    iris.scale.set(1, 1, 0.45);
    tete.add(iris);
    const sourcil = new THREE.Mesh(new THREE.BoxGeometry(0.06, 0.013, 0.014), matCheveux);
    sourcil.position.set(cote * 0.067, 0.066, 0.146);
  ...
  bouche.position.set(0, -0.078, 0.145);
  tete.add(bouche);
